...
- (BTCTransactionBuilderResult*) buildTransaction:(NSError**)errorOut {
    ...
    // Enhance privacy by randomizing outputs so change address cannot be reliably determined
    ...
    // Add txoutputs to transaction
    for (BTCTransactionOutput* txout in txoutputs) {
        // Don't add change value to outputsAmount as it hasn't been calculated yet
        if (txout.value != BTC_MAX_MONEY)
